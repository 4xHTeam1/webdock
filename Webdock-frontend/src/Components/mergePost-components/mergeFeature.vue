<template>
    <div :class="`Feature_Container ${active ? 'Feature_Active' : ''}`">
        <img src="../../Assets/icons/check.svg" alt="check" class="Feature_Selected" v-if="active" />
        <div class="Feature_InfoContainer">
            <p :class="`Feature_Title ${active ? 'Feature_Text_Active' : ''}`">{{ feature.title }}</p>
            ...
            <p :class="`Feature_Description ${active ? 'Feature_Text_Active' : ''} `">{{ feature.description }}</p>
        </div>
    </div>
</template>
