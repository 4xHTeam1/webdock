      ...
      <div class="postUserInfo">
        <div class="usersAvatar">
          ...
          <img
            ...
            alt="webdock admin"
            v-if="comment.user.role.toLowerCase() === 'admin'"
            class="admin_logo"
          />
        </div>
        <div
          class="userName"
          :style="{
            color: comment.user.role.toLowerCase() === 'admin' ? '#018647' : '',
          }"
        >
          {{
            comment.user.role.toLowerCase() === "admin"
              ? comment.user.name + " from Webdock"
              : comment.user.name
          }}
        </div>
      </div>
    ...
    <div class="bottomContainer">
      ...
      <div class="commentReplyContainer" v-if="showCommentReplyContainer && auth.user !== null" >
        ...
        >
          <div
            class="submitBtn"
            :class="{ btnActive: this.commentMessage !== '' }"
...
export default {
  ...
  methods: {
    ...
    handleSubmitReply() {
      if (!this.auth.user || this.commentMessage === "") {
        return;
      }
